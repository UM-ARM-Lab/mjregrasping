...
def prm(phy: Physics,
    ...
    # add the start
    # nodes are indices into the qs list
    G.add_node(0)

    # sample a goal and add it
    goal_q = goal_sampler(phy, goal)
    qs.append(goal_q)
    G.add_node(len(qs) - 1)

    # terminate when the graph has one connected component, which means that the start and goal are connected
    while not is_connected(G):
        # sample a new configuration
        q_rand = uniform_rand_q(phy.m, rng)

        # check if the configuration is valid
        if not state_checker(phy, q_rand, viz):
            continue

        # find the k nearest neighbors in the graph, before adding the new config
        perf
        kd = KDTree(qs)
        _, neighbor_ids = kd.query(q_rand[None], k=min(knn, len(qs)))

        # add the new random valid config to the graph
        qs.append(q_rand)
        q_rand_id = len(qs) - 1
        G.add_node(q_rand_id)

        neighbor_ids = neighbor_ids[0]

        for neighbor_id in neighbor_ids:
            # take small steps towards the neighbor and check for collisions
            q_neighbor = qs[neighbor_id]
            d_total = np.linalg.norm(q_rand - q_neighbor)
            n_step = max(int(d_total / dq_step_size), 1)

            # interpolate from q_rand to q_neighbor
            neighbor_connected = True
            q_new = None
            for d in np.linspace(dq_step_size, d_total, n_step):
                q_new = q_rand + d * (q_neighbor - q_rand) / d_total

                if viz.p.prm_q_new:
                    set_q_and_do_fk(phy, q_new)
                    viz.viz(phy, is_planning=True)

                if not state_checker(phy, q_new, viz):
                    neighbor_connected = False
                    break

            if neighbor_connected:
                G.add_edge(q_rand_id, neighbor_id)
            elif q_new is not None:
                qs.append(q_new)
                q_new_id = len(qs) - 1
                G.add_node(q_new_id)
                G.add_edge(q_rand_id, q_new_id)
            ...
            # if the connection didn't make it the whole way, then try the reverse: from q_neighbor to q_rand
            q_new = None
            for d in np.linspace(dq_step_size, d_total, n_step):
                q_new = q_neighbor + d * (q_rand - q_neighbor) / d_total
            ...
            if q_new is not None:
                ...
                q_new_id = len(qs) - 1
                G.add_node(q_new_id)
                G.add_edge(neighbor_id, q_new_id)
