...
def mjcf_scene_to_sdf(model_filename: pathlib.Path, res: float, xmin: float, xmax: float, ymin: float, ymax: float,
                      zmin: float, zmax: float):
    m = mujoco.MjModel.from_xml_path(str(model_filename))
    d = mujoco.MjData(m)
    phy = Physics(m, d)
