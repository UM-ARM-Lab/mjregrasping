...
void parallel_rollout(ctpl::thread_pool &p, mjModel const *model, mjData *data, std::vector<mjData *> datas, std::vector<std::vector<Control>> const &controls)
{
    ...
    for (auto i = 0; i < controls.size(); ++i)
    {
        ...
        results.emplace_back(p.push(
            [&model, data_i = std::move(data_i), &controls_i](int)
            {
