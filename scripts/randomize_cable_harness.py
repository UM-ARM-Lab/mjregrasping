...
def main():
    ...
    for i in range(10):
        ...
        phy_path = root / f"{scenario.name}_{i}_phy.pkl"
        with phy_path.open('wb') as f:
            pickle.dump(phy, f)

        sdf_path = root / f"{scenario.name}_{i}.sdf"
        get_sdf(sdf_path, phy, 0.01, xmin=-1.0, xmax=1.0, ymin=0.2, ymax=1.0, zmin=-0.45, zmax=1.1)
        ...
        trial_path = root / f"{scenario.name}_{i}.pkl"
